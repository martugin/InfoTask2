...
namespace BaseLibrary
{
    ...
    public class BulkSql : IRecordAdd
    {
        public BulkSql(SqlProps props, string tabl)
        {
            _tabl = tabl;
            _props = props;
            //Получение списка колонок таблицы
            using ( var rec = new ReaderAdo(props, "SELECT fld.name, fld.length FROM sysobjects tab, syscolumns fld WHERE tab.id=fld.id And tab.name='" + tabl + "'"))
            {
                int i = 0;
                while (rec.Read())
                {
