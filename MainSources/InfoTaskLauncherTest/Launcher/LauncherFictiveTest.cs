...
namespace InfoTaskLauncherTest
{
    ...
    public class LauncherFictiveTest
    {
        ...
        public void FictiveSimple()
        {
            ...
            var sigInt = con.AddSignal("Out1.Int", "int", "Список со срезом", "NumObject=1;ValuesInterval=30000", "", "Signal=Int");
            Assert.AreEqual("Out1.Int", sigInt.Code);
            Assert.AreEqual("Целое", sigInt.DataType);
            var sigReal = con.AddSignal("Out1.Real", "real", "Список со срезом", "NumObject=1;ValuesInterval=30000", "", "Signal=Real");
            Assert.AreEqual("Out1.Real", sigReal.Code);
            Assert.AreEqual("Действ", sigReal.DataType);
            var sigTime = con.AddSignal("Out1.Time", "time", "Список со срезом", "NumObject=1;ValuesInterval=30000", "", "Signal=Time");
            Assert.AreEqual("Out1.Time", sigTime.Code);
            Assert.AreEqual("Время", sigTime.DataType);
            var sigString = con.AddSignal("Out1.String", "string", "Список со срезом", "NumObject=1;ValuesInterval=30000", "", "Signal=String");
            Assert.AreEqual("Out1.String", sigString.Code);
            Assert.AreEqual("Строка", sigString.DataType);
            var sigValue = con.AddSignal("Out2.Value", "real", "Список со срезом", "NumObject=2;ValuesInterval=10000", "", "Signal=Value");
            Assert.AreEqual("Out2.Value", sigValue.Code);
            Assert.AreEqual("Действ", sigValue.DataType);
            var sigState = con.AddSignal("Out2.State", "int", "Список со срезом", "NumObject=2;ValuesInterval=10000", "", "Signal=State");
        ...
        public void Fictive()
        {
            ...
            Assert.AreEqual("Out1.Bool", sigBool.Code);
            Assert.AreEqual("Логич", sigBool.DataType);
            var sigInt = con.AddSignal("Out1.Int", "int", "Список со срезом", "Table=MomValues;ObjectCode=Ob1", "", "Signal=Int");
            Assert.AreEqual("Out1.Int", sigInt.Code);
            Assert.AreEqual("Целое", sigInt.DataType);
            var sigReal = con.AddSignal("Out1.Real", "real", "Список со срезом", "Table=MomValues;ObjectCode=Ob1", "", "Signal=Real");
            Assert.AreEqual("Out1.Real", sigReal.Code);
            Assert.AreEqual("Действ", sigReal.DataType);
            var sigTime = con.AddSignal("Out1.Time", "time", "Список со срезом", "Table=MomValues;ObjectCode=Ob1", "", "Signal=Time");
            Assert.AreEqual("Out1.Time", sigTime.Code);
            Assert.AreEqual("Время", sigTime.DataType);
            var sigString = con.AddSignal("Out1.String", "string", "Список со срезом", "Table=MomValues;ObjectCode=Ob1", "", "Signal=String");
            Assert.AreEqual("Out1.String", sigString.Code);
            Assert.AreEqual("Строка", sigString.DataType);
            var sigValue = con.AddSignal("Out2.Value", "real", "Список со срезом", "Table=MomValues;ObjectCode=Ob2", "", "Signal=Value");
            Assert.AreEqual("Out2.Value", sigValue.Code);
            Assert.AreEqual("Действ", sigValue.DataType);
            var sigState = con.AddSignal("Out2.State", "int", "Список со срезом", "Table=MomValues;ObjectCode=Ob2", "", "Signal=State");
            Assert.AreEqual("Out2.State", sigState.Code);
            Assert.AreEqual("Целое", sigState.DataType);
            var sigValueX = con.AddSignal("OutX.Value", "real", "Список со срезом", "Table=MomValues2;ObjectCode=ObX", "", "Signal=Value");
            Assert.AreEqual("OutX.Value", sigValueX.Code);
            Assert.AreEqual("Действ", sigValueX.DataType);
            var sigValue2X = con.AddSignal("OutX.Value2", "real", "Список со срезом", "Table=MomValues2;ObjectCode=ObX", "", "Signal=Value2");
